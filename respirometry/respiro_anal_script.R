...
dat = lapply(f, function(i)
{
  ...
  species  <- output3[2] # get species (M or L)
  clam <- output3[4] # get clam name
...
dat <- unite(data=dat,"ID",c(species,clam),sep="_",remove=FALSE)
...
getwd(); setwd('..'); setwd('constraints'); getwd()
write.xlsx(x=dat_nested$ID_sort, file="constraints_20220615_T0.xlsx", sheetName = "20220615_T0", col.names = TRUE, row.names = TRUE, append = FALSE)


# Plot the lines one by one to find ones to trash. Add start and stop to for recalcs in constraints tab of MR_output.

getwd(); setwd('..'); setwd('QC_plots/20220614_T0'); getwd()
...
constraints <- read_excel("constraints_20220615_T0.xlsx", sheet = "20220615_T0", col_names = TRUE)
